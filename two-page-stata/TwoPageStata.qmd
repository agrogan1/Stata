...
Like many other statistical programs, Stata makes a large distinction between variables that are coded as *numeric*, and variables that are coded as *strings*. `describe x` will help you to ascertain the variable type. `encode x, generate(x_NUMERIC)` is often useful to create a *numeric* version of *string* variables.
