...
## Categorical by Categorical (`graph bar, over(x) over(y)`) [^asyvars1]

[^asyvars1]: You can also use `asyvars` with categorical by categorical bar graphs
...
## Continuous by Categorical (`graph bar y, over(x) scheme(...)`)[^asyvars2]

[^asyvars2]: In these graphs I again use `asyvars` to give different colors to the bars on the graph.
